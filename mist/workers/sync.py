...
def get_uri(**kwargs):
    ...
    path = kwargs['path']
    if kwargs.get('uuid'):
        path = _get_abs_path(host, kwargs['uuid'], path)
        if not path:
            return
    return 'rsync://%s:%s@%s%s:%s' % (host.username, host.password, host.host, path, host.port)
...
def process_sync(sync_id):
    ...
    if Transfer.find_one({'sync_id': sync['_id'], 'finished': None}):
        set_retry(sync)
        return
    src = get_uri(**sync['src'])
    if not src:
        set_retry(sync)
        return
    dst = get_uri(**sync['dst'])
    if not dst:
        set_retry(sync)
        return
